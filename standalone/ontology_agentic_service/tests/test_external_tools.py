...
from AgentsRegistry.CodedTools import external_context_graph_search as context_tools
from AgentsRegistry.CodedTools import external_registered_ontologies as registered_tools
...
def test_registered_ontologies_uses_configured_endpoint(monkeypatch):
    ...
    monkeypatch.setattr(registered_tools, "urlopen", fake_urlopen)
    assert registered_tools.run_external_registered_ontologies() == {"items": []}
    assert captured == {
...
def test_context_search_posts_json(monkeypatch):
    ...
    monkeypatch.setenv("ONTOLOGY_EXTERNAL_API_ENABLED", "true")
    monkeypatch.setattr(context_tools, "urlopen", fake_urlopen)
    assert context_tools.run_external_context_graph_search("pump", "ex") == {"matches": [1]}
...
def test_external_target_and_input_are_validated(monkeypatch):
    ...
    with pytest.raises(ValueError, match="HTTP"):
        registered_tools.run_external_registered_ontologies()
    with pytest.raises(ValueError, match="search"):
        context_tools.run_external_context_graph_search("  ")
