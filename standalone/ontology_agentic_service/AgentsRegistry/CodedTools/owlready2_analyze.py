"""Self-contained IIF coded tool: owlready2_analyze."""
...
def run_owlready2_analyze(
    path: str,
    run_reasoner: bool = False,
    reasoner: Literal["hermit", "pellet"] = "hermit",
    infer_property_values: bool = False,
) -> dict[str, Any]:
    """Load an ontology in an isolated Owlready2 World and optionally reason."""
    ...
    try:
        ...
        raise RuntimeError("Install owlready2>=0.48 to use this tool") from exc
    world = World()
    try:
        with ontology_path.open("rb") as source:
            ontology = world.get_ontology(ontology_path.as_uri()).load(fileobj=source, only_local=True)
        before = {
            "classes": len(list(ontology.classes())), "object_properties": len(list(ontology.object_properties())),
        ...
        if run_reasoner:
            if reasoner == "pellet":
                sync_reasoner_pellet(world, infer_property_values=infer_property_values, infer_data_property_values=infer_property_values, debug=0)
            else:
        ...
        return {
            "path": str(ontology_path), "engine": "owlready2", "world_scope": "isolated",
            "remote_imports_allowed": False, "ontology_iri": ontology.base_iri,
            "reasoning_run": run_reasoner, "reasoner": reasoner if run_reasoner else None,
            "before_reasoning": before, "after_reasoning": after,
...
owlready2_analyze = FunctionTool.from_defaults(name="owlready2_analyze", fn=run_owlready2_analyze)
