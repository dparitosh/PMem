...
try:
    from .ap239_mapper_service import AP239MapperService, AP239DomainPipeline
...
try:
    from .multi_domain_pipeline_controller import (
...
try:
    ...
    try:
        from .plmxml_parser import parse_plmxml_file, PlmxmlDocument
    ...
    try:
        from .step_parser import parse_step_with_pmi, StepP21Entity, StepDimension, StepDatum, StepGeometricTolerance
...
class DataImportService:
    ...
    @classmethod
    def _safe_cypher_identifier(cls, value: Any, fallback: str = 'Entity') -> str:
    ...
    @classmethod
    async def process_file(cls, file_content: bytes, filename: str, ontology_mapping: str = '') -> str:
        """
    ...
    async def _parse_stage(cls, task_id: str, file_type: str, file_path: str) -> Dict[str, Any]:
        ...
        if file_type == 'plmxml':
            parsed = cls._parse_plmxml(file_path)
        elif file_type == 'step':
            parsed = cls._parse_step(file_path)
        elif file_type == 'xsd':
            parsed = cls._parse_xsd(file_path)  # NEW: XSD schema parsing
        elif file_type == 'xmi':
            parsed = cls._parse_xmi(file_path)  # NEW: XMI (UML/MOF) parsing
        elif file_type == 'xml':
            parsed = cls._parse_xml(file_path)
        elif file_type == 'excel':
            parsed = cls._parse_excel(file_path)
        elif file_type == 'csv':
            parsed = cls._parse_csv(file_path)
        elif file_type == 'json':
            parsed = cls._parse_json(file_path)
        elif file_type == 'html':
            parsed = cls._parse_html(file_path)
        elif file_type == 'owl':
            parsed = cls._parse_ontology(file_path)
        else:
            parsed = cls._parse_xml(file_path)  # Fallback to generic XML
    ...
    async def _map_stage(cls, task_id: str, parsed_data: Dict[str, Any], ontology_mapping: str = '') -> Dict[str, Any]:
        ...
        try:
            from .ontology_mapper_service import OntologyMapperService
    ...
    async def _transform_stage(cls, task_id: str, parsed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_entities > MAX_ENTITIES:
                logger.warning(f"ENTITY LIMIT: {total_entities} entities exceed limit of {MAX_ENTITIES}")
            ...
            if dropped_relationships:
                logger.warning(f"DROPPED RELATIONSHIPS: {len(dropped_relationships)} relationships excluded")
            ...
            logger.info(f"Transformed {len(ontology_entities)} entities with {len(relationships)} relationships")
            if dropped_relationships:
                logger.info(f"  ({len(dropped_relationships)} orphaned relationships excluded)")
    ...
    async def _ingest_stage(cls, task_id: str, transformed_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for entity in transformed_data.get('ontology_entities', []):
                try:
                    label = cls._safe_cypher_identifier(
                        entity.get('mapped_type', entity.get('original_type', 'Entity')),
                        fallback='Entity',
                    )
                    ...
                    result = graph.query(
                        cypher,
                        {
                            ...
                            'attributes_json': json.dumps(entity.get('attributes', {}), default=str),
            ...
            for rel in transformed_data.get('relationships', []):
                try:
                    ...
                    if source_id in entity_id_map and target_id in entity_id_map:
                        ...
                        rel_type = cls._safe_cypher_identifier(
                            rel.get('relationship') or rel.get('type') or 'RELATES_TO',
                            fallback='RELATES_TO',
                        ).upper()
                        cypher = f"""
                        MATCH (a), (b)
    ...
    def _parse_plmxml(cls, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"PLMXML parsed: {len(entities)} entities, {len(relationships)} relationships")
    ...
    def _parse_step(cls, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"STEP parsed: {len(entities)} entities (including {len(doc.entities)} core entities, {len(doc.dimensions)} dimensions, {len(doc.geometric_tolerances)} tolerances)")
    ...
    def _parse_xsd(cls, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"XSD parsed: {len(entities)} schema elements")
            ...
            if constraints_count > 0:
                logger.info(f"  Extracted {constraints_count} constraint definitions (patterns, lengths, ranges)")
    ...
    def _parse_xmi(cls, file_path: str) -> Dict[str, Any]:
        ...
        try:
            if XMI_PARSER_AVAILABLE:
                ...
                logger.info(f"XMI parsed: {len(entities)} entities, {len(relationships)} relationships")
                return {
                    'entities': entities,
                    'relationships': relationships,
                    'format': 'xmi',
    ...
    def _parse_csv(cls, file_path: str) -> Dict[str, Any]:
        """Parse CSV file into row entities."""
        try:
            ...
            for row_idx, row in enumerate(reader, 1):
                cleaned = {
                    str(k).strip() if k is not None else 'column': (v.strip() if isinstance(v, str) else v)
                    for k, v in row.items()
                }
                if not any(v not in (None, '') for v in cleaned.values()):
                    continue
                row_id = cleaned.get('id') or cleaned.get('ID') or cleaned.get('name') or cleaned.get('Name') or f"CSV_ROW_{row_idx}"
                entities.append({
                    'type': 'CSVRow',
                    'id': str(row_id),
                    'name': str(cleaned.get('name') or cleaned.get('Name') or row_id),
                    'attributes': cleaned,
                })
            ...
            return {'entities': [], 'relationships': [], 'format': 'csv', 'error': str(e)}

    @classmethod
    def _parse_json(cls, file_path: str) -> Dict[str, Any]:
        """Parse JSON array/object into entities."""
        try:
            data = json.loads(Path(file_path).read_text(encoding='utf-8-sig'))
            if isinstance(data, list):
                items = data
            elif isinstance(data, dict):
                items = data.get('items') or data.get('records') or data.get('data') or [data]
            else:
                items = []
            ...
            for idx, item in enumerate(items, 1):
                if not isinstance(item, dict):
                    item = {'value': item}
                row_id = item.get('id') or item.get('ID') or item.get('name') or item.get('Name') or f"JSON_ITEM_{idx}"
                entities.append({
                    'type': item.get('type') or item.get('label') or 'JSONRecord',
                    'id': str(row_id),
                    'name': str(item.get('name') or item.get('Name') or row_id),
                    'attributes': item,
                })
                for key, value in item.items():
                    if key.endswith('_id') and value:
                        relationships.append({'source': str(row_id), 'target': str(value), 'type': key.upper()})
            ...
            return {'entities': [], 'relationships': [], 'format': 'json', 'error': str(e)}

    @classmethod
    def _parse_html(cls, file_path: str) -> Dict[str, Any]:
        """Parse HTML headings/tables/links into lightweight entities."""
        try:
            from bs4 import BeautifulSoup

            soup = BeautifulSoup(Path(file_path).read_text(encoding='utf-8', errors='replace'), 'html.parser')
            entities = []
            for idx, tag in enumerate(soup.find_all(['h1', 'h2', 'h3', 'table', 'a'])[:500], 1):
                text = tag.get_text(' ', strip=True)
                if not text and tag.name != 'table':
                    continue
                ent_id = tag.get('id') or tag.get('name') or f"HTML_{tag.name}_{idx}"
                entities.append({
                    'type': f"HTML_{tag.name.upper()}",
                    'id': ent_id,
                    'name': text[:120] or ent_id,
                    'attributes': {
                        'tag': tag.name,
                        'text': text,
                        'href': tag.get('href', ''),
            ...
            return {'entities': entities, 'relationships': [], 'format': 'html', 'row_count': len(entities)}
        except Exception as e:
            logger.exception("HTML parsing failed")
            return {'entities': [], 'relationships': [], 'format': 'html', 'error': str(e)}

    @classmethod
    def _parse_ontology(cls, file_path: str) -> Dict[str, Any]:
        """Parse an OWL/RDF/Turtle file as imported ontology text."""
        try:
            ...
            for idx, line in enumerate(text.splitlines(), 1):
                ...
                if not stripped or stripped.startswith('@prefix') or stripped.startswith('#'):
                    continue
                subject = stripped.split()[0] if stripped.split() else f"TTL_{idx}"
                entities.append({
                    'type': 'OntologyStatement',
                    'id': f"TTL_{idx}",
                    'name': subject,
                ...
                if len(entities) >= 500:
                    break
            return {'entities': entities, 'relationships': [], 'format': 'ontology', 'row_count': len(entities)}
        except Exception as e:
            logger.exception("Ontology parsing failed")
            return {'entities': [], 'relationships': [], 'format': 'ontology', 'error': str(e)}
    ...
    def _parse_excel(cls, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for row_idx, row in df.fillna('').iterrows():
                entity_data = {str(k): v for k, v in row.to_dict().items()}
                if not any(v not in (None, '') for v in entity_data.values()):
                    continue
                row_id = entity_data.get('id') or entity_data.get('ID') or entity_data.get('name') or entity_data.get('Name') or f"ROW_{row_idx + 1}"
                entities.append({
                    'type': 'DataRow',
                    'id': str(row_id),
                    'name': str(entity_data.get('name') or entity_data.get('Name') or row_id),
            ...
            return {'entities': [], 'relationships': [], 'format': 'excel', 'error': str(e)}
    ...
    def get_task_status(cls, task_id: str) -> Dict[str, Any]:
        ...
        status = {
            ...
            'committed': task.get('committed', False),
        }
    ...
    @classmethod
    def confirm_import(cls, task_id: str) -> Dict[str, Any]:
        """Confirm a pipeline import that has already loaded data."""
        if task_id not in import_tasks:
            raise KeyError(f"Task {task_id} not found")
        task = import_tasks[task_id]
        if task.get('status') == 'failed':
            raise ValueError(task.get('error') or 'Cannot confirm a failed import')
        task['committed'] = True
        task['message'] = 'Import confirmed'
        return {
            'task_id': task_id,
