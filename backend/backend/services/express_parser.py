...
def _parse_entity_block(name: str, body: str) -> ExpressEntity:
    ...
    if re.search(r"^\s*ABSTRACT\s*$", body, re.MULTILINE | re.IGNORECASE):
...
def _parse_select_type(name: str, type_body: str) -> ExpressSelectType:
    ...
    # Check for BASED_ON
    based_on_match = re.search(r"SELECT\s+BASED_ON\s+(\w+)\s+WITH\s*\(([^)]+)\)", type_body, re.IGNORECASE | re.DOTALL)
    based_on = based_on_match.group(1).strip() if based_on_match else None
    
    # Extract SELECT members
    members: List[str] = []
    select_match = re.search(r"SELECT\s*\(([^)]+)\)", type_body, re.IGNORECASE | re.DOTALL)
    if select_match:
        raw = select_match.group(1)
        members = [v.strip() for v in raw.split(",") if v.strip()]
    elif based_on_match:
        # For BASED_ON, also parse the WITH clause members
        raw = based_on_match.group(2)
        members = [v.strip() for v in raw.split(",") if v.strip()]
...
def _build_attr_lookup(entity: ExpressEntity) -> Dict[str, str]:
    ...
    return {a.name.lower(): a.name for a in entity.attributes}
...
def _translate_sizeof_patterns(entity: ExpressEntity, expr: str, ns: str) -> Optional[str]:
    ...
    if m:
        a1 = _resolve_attr_ref(m.group(1), attr_lookup)
        op = m.group(2)
        a2 = _resolve_attr_ref(m.group(3), attr_lookup)
        if a1 and a2:
            c1 = "?c1"
            c2 = "?c2"
            p1 = f"<{ns}{entity.name}_{a1}>"
            ...
            return (
                "SELECT $this WHERE {\n"
                f"        {{ SELECT $this (COUNT(?x1) AS {c1}) WHERE {{ OPTIONAL {{ $this {p1} ?x1 . }} }} GROUP BY $this }}\n"
                f"        {{ SELECT $this (COUNT(?x2) AS {c2}) WHERE {{ OPTIONAL {{ $this {p2} ?x2 . }} }} GROUP BY $this }}\n"
...
def _translate_typeof_membership_patterns(entity: ExpressEntity, expr: str, ns: str) -> Optional[str]:
    ...
    if " IN TYPEOF(" in normalized and ".Definitional" in normalized and normalized.upper().startswith("NOT"):
    ...
    if " IN TYPEOF(" in normalized and " OR " in normalized:
        types = re.findall(r"'([A-Za-z0-9_]+)'\s+IN\s+TYPEOF\(\s*([^)]+?)\s*\)", normalized, flags=re.IGNORECASE)
        if len(types) >= 2:
            t1, obj1 = types[0]
            t2, obj2 = types[1]
...
def _translate_query_patterns(entity: ExpressEntity, expr: str) -> Optional[str]:
    ...
    if (
        "SIZEOF( QUERY(xi <* RotationMatrix" in normalized
        and "(-1.0 > xj) OR (xj > 1.0)" in normalized
    ):
        return (
            "SELECT $this WHERE {\n"
    ...
    if (
        "SIZEOF(QUERY( ac <* AssemblyContext" in normalized
        and "WiringHarnessAssemblyDesign" in normalized
        and "TYPEOF(ac\\AssemblyOccurrenceRelationship.Relating)" in normalized
    ):
        return (
            "SELECT $this WHERE {\n"
...
def _translate_where_rule_to_sparql(
    ...
    if re.search(r"\b(QUERY|UNIQUE|IN)\b", expr, flags=re.IGNORECASE):
        return None
    ...
    lhs_canon = _resolve_attr_ref(lhs_raw, attr_lookup)
    ...
    if rhs_kind in {"var", "varpath"}:
        rhs_canon = _resolve_attr_ref(rhs_raw, attr_lookup)
