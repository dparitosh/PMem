...
class Neo4jSchemaCleaner:
    ...
    def delete_all_nodes_and_relationships(self) -> Tuple[bool, str]:
        ...
        try:
            with self.driver.session() as session:
                ...
                rel_deleted = rel_result.single()["count"] if rel_result.single() else 0
                
                node_result = session.run("MATCH (n) DELETE n RETURN count(n) as count")
                node_deleted = node_result.single()["count"] if node_result.single() else 0
    ...
    def create_indexes(self) -> Tuple[bool, str]:
        ...
        indexes_to_create = [
            "CREATE INDEX idx_entity_id IF NOT EXISTS FOR (n) ON (n.id)",
            "CREATE INDEX idx_entity_type IF NOT EXISTS FOR (n) ON (n.type)",
            "CREATE INDEX idx_entity_name IF NOT EXISTS FOR (n) ON (n.name)",
            "CREATE FULLTEXT INDEX idx_fulltext IF NOT EXISTS FOR (n) ON EACH [n.name, n.description]",
        ]
