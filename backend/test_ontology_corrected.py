...
test("1. Available Static Ontologies (Before Any Import)")
try:
...
test("2. Map Entity to AP239 (Correct Request Format)")
try:
...
test("3. Query OntologyMetadata from Neo4j SPDMS")
try:
...
test("4. Total Data in SPDMS Database")
try:
...
test("5. AP239 Data Dictionary Accessibility")
try:
...
test("6. AP239 to PLMXML Mappings Accessibility")
try:
...
test("7. Multi-Domain Pipeline Configuration")
try:
...
test("8. Railway Domain Pipeline Configuration")
try:
...
# SUMMARY
test("SUMMARY - Ontology Creation and Accessibility")
print(f"""
