...
app.include_router(create_odata_catalog_router(service_name="DEPODataCatalog", capabilities=[ServiceCapability("Data products", "/api/v1/catalog/products", description="Discover governed product versions"), ServiceCapability("Product history", "/api/v1/catalog/products/{product_id}", description="Read immutable product versions and latest pointer"), ServiceCapability("Register product version", "/api/v1/catalog/products/{product_id}/versions/{version}", "PUT", "Register an approved immutable product version")]))
