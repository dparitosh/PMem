...
test('audit graph explorer current UI state', async ({ page }) => {
  const logs = [];
  ...
  await page.screenshot({ path: '../tmp-graph-explorer-audit.png', fullPage: true });
  ...
  console.log('SVG text sample:', JSON.stringify(textLabels.slice(0, 25), null, 2));
  console.log('SVG circle count:', circleCount);
  console.log('SVG path count:', pathCount);
  console.log('SVG line count:', lineCount);
  console.log('Body text sample:', canvasText.slice(0, 2500));
  console.log('Console logs:', JSON.stringify(logs.slice(-30), null, 2));
});
