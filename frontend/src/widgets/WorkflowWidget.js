...
import { CheckCircle2, Database, FileInput, Network, Workflow } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div style={{ color: widgetColors.muted, fontSize: 10, lineHeight: 1.3 }}>
        <strong>Input:</strong> {formatValue(workflow?.inputs)}
      </div>
      <div style={{ color: widgetColors.muted, fontSize: 10, lineHeight: 1.3 }}>
        <strong>Output:</strong> {formatValue(workflow?.outputs)}
      </div>
