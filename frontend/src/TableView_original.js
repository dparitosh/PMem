...
//             return (
//                 <tr key={elementId}> 
//                  <td>{treeViewData(sourceNode[0], sourceNode[0].klusa_name ?? sourceNode[0].project_name ?? sourceNode[0]?.milestone_abbreviation ?? sourceNode[0]?.[`${sourceNode[0]?.type}_name`] ?? sourceNode[0]?.label)}</td>
//                  <td title={type} style={{paddingTop:'20px'}}>{type.toLowerCase().replace('has_', '')}</td>
//                  <td>{treeViewData(targetNode[0], targetNode[0]?.[`${targetNode[0]?.type}_name`] ?? targetNode[0]?.label)}</td>
//               </tr>
//            )
//          })
//     }
