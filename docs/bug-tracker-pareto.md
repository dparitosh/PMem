...
| WU-02 | Where Used | Upward traversal assumes parents will always be returned in a single shape. | Parent hierarchy can truncate or skip nodes. | `WhereUsedView.js:196-301` | P1 | Fixed | Where Used now calls the shared traversal API wrapper and normalizes record-style and graph-style traversal responses before building hierarchy. |
