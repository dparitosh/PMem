...
### 2026-07-04 OSLC service provider and Teamcenter LDS release note

- Updated `docs/oslc-gap-audit.md` to remove stale pre-implementation statements and document the current OSLC-aligned runtime surface.
- Confirmed DEPO exposes an OSLC Service Provider Catalog at `/oslc/catalog` and provider document at `/oslc/providers/depo` for Teamcenter Linked Data Service Framework registration.
- Documented customer registration settings: `OSLC_ENABLED`, `OSLC_TRS_ENABLED`, `OSLC_BASE_URL`, `OSLC_PROVIDER_ID`, `OSLC_PROVIDER_TITLE`, `OSLC_MAX_PAGE_SIZE`, and `ALLOWED_ORIGINS`.
- Clarified release language: DEPO is OSLC-aligned for read/query/shape/dictionary/taxonomy/TRS discovery, but is not yet a full OSLC-certified write/update provider.
- Documented Teamcenter integration split: use OSLC endpoints for linked-data discovery and use `/recommendations/change-impact` or async `/chat/jobs` for AI/GraphRAG workflows.

### 2026-07-04 AP242 and OSLC AM domain metadata update

- OSLC Service Provider now advertises explicit `ap242` and `oslc_am` domain scopes for Teamcenter Linked Data Service Framework discovery.
...
- Made import and ontology request timeouts configurable through `IMPORT_REQUEST_TIMEOUT_SECONDS` and `ONTOLOGY_REQUEST_TIMEOUT_SECONDS`, defaulting large import requests to 3600 seconds.
- Added customer-facing large-import `.env` settings for Neo4j import query timeout and node/relationship batch sizes.
- XMI parser compile and focused XMI parser tests passed after the change.

### 2026-07-04 SysML v2 API client audit

- Audited the `Systems-Modeling/SysML-v2-API-Python-Client` repository for fit with DEPO.
- Outcome: useful as a future optional live SysML v2 repository connector, not a replacement for the current XMI/MDXML parser or large-file timeout fixes.
- Added `docs/sysml-v2-api-client-audit.md`, release-guide notes, and disabled-by-default readiness endpoints under `/api/v1/sysml-v2/*` to prevent customer messaging from overstating current support.
- Open follow-up: implement actual SysML v2 repository synchronization only after a real customer SysML v2 API server, authentication model, OpenAPI compatibility, and licensing posture are confirmed.

### 2026-07-05 KerML audit

- Reviewed KerML as the semantic kernel foundation for SysML v2.
- Current code has no explicit `.kerml` parser, KerML route, or KerML-to-OWL/Neo4j projection service.
- Added `docs/kerml-audit.md` and updated the SysML v2 readiness integration plan to mark KerML as planned, not implemented.
- Release guidance remains: XMI/MDXML is the supported MBSE file path; SysML v2/KerML are readiness/planned connector areas until customer server/files and parser strategy are validated.

### 2026-07-05 SysML generation audit

- Confirmed the SysML v2 API client may help publish generated model content to a live SysML v2 repository, but it is not a local `.sysml` / `.kerml` generator.
- Current DEPO runtime does not generate SysML v2 textual files. Add a deterministic `SysMLGenerationService` before exposing generation in UI or API.
- LLM output should be treated as draft assistance only; final SysML/KerML should be generated by rules/templates and validated before export or repository commit.

### 2026-07-05 Large XMI parser audit update

- Audited `backend/Services/xmi_parser.py`, `backend/Services/data_import_service.py`, and `backend/Services/unified_data_import.py` for large XMI processing.
- Patched XMI semantic relationship extraction to build class/property/association/dependency/abstraction/interface/stereotype/SysML buckets in one `root.iter()` scan instead of repeated full-tree scans.
- Neo4j commit path already uses configurable node and relationship batch sizes: `IMPORT_WRITE_BATCH_SIZE`, `IMPORT_LINK_BATCH_SIZE`, and `IMPORT_COMMIT_QUERY_TIMEOUT`.
- Remaining limitation: XMI parsing still uses `lxml.etree.parse()` and keeps the full XML tree in memory. True streaming XMI parsing remains a future P0/P1 performance hardening task for very large `.xmi`/`.mdxml` files.
- Validation: `python -m py_compile backend\Services\xmi_parser.py backend\Services\data_import_service.py backend\Services\unified_data_import.py`; focused XMI parser pytest passed.

### 2026-07-05 Frontend UI component audit

- Audited frontend component inventory, page routing, navigation, API config, and major UI component risks.
- Added `docs/frontend-ui-component-audit.md`.
- Confirmed Workspace is removed from navigation and aliases to Graph Explorer.
- Confirmed `GraphHEB.js`, `OntologyMapper.js`, and `DataImportPipeline.js` remain the dominant release-risk components by size and responsibility coupling.
