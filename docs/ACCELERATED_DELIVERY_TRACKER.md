...
| P1 | Standards-to-CEIM mapping | Complete | AP242, QIF and ReqIF mapping packs normalize with provenance and can use governed publication. The NIST AP242 QIF sample passes QIF 3.0 XSD, declared CEIM mapping coverage, CEIM SHACL, and isolated OpenAPI normalization. Its 174 official quality findings remain retained source-quality evidence, not suppressed or converted into graph assertions. |
...
| P2 | Distributed semantic analytics | In progress | `rdf-quality-statistics` reports lexical integrity and predicate distribution. `rdf-deduplicate-serialize` performs bounded Spark validation, distributed deduplication and deterministic N-Triples serialization into immutable accepted/rejected artifacts without a graph write. `enrich-document-evidence` turns content-addressed document chunks into a versioned Document/DocumentChunk technical-graph proposal with normalized text, chunk digests and provenance; it intentionally performs no OCR, model-based NER, embeddings or graph write. The read-only GraphQL boundary exposes durable job-run and data-product manifests but cannot submit Spark work or mutate the graph. Distributed OWL reasoning/rule materialization and semantic ML require concrete governed product use cases and remain pending. |
